...
export default function AdminDashboard({ onExitAdmin, showToast, sectionsConfig: propSectionsConfig, onUpdateSectionsConfig, settings: propSettings, onUpdateSettings }) {
  ...
  // Dynamic Hero Slides, Instagram Feed, Editorial Promo & Brand Story States
  ...
  const [showInstagramPostModal, setShowInstagramPostModal] = useState(false);
  const [editingInstagramPost, setEditingInstagramPost] = useState(null);
  const [instagramPostForm, setInstagramPostForm] = useState({
    title: '',
    image_url: '',
  ...
  const [editorialPromoForm, setEditorialPromoForm] = useState({
  ...
  const fetchTabData = async (tab) => {
    ...
    try {
      if (tab === 'media') {
        ...
        const [hero, slds] = await Promise.all([
          safeFetchJson('/api/hero-config'),
          safeFetchJson('/api/admin/hero-slides')
        ]);
        if (hero && hero.id) setHeroConfig(hero);
        if (slds && Array.isArray(slds)) setHeroSlides(slds);
      } else if (tab === 'theme') {
      ...
      } else if (tab === 'sections' || tab === 'editorial') {
        const [sec, prm, sty, igs] = await Promise.all([
          safeFetchJson('/api/sections-config'),
          safeFetchJson('/api/editorial-promo'),
          safeFetchJson('/api/brand-story'),
          safeFetchJson('/api/admin/instagram-posts')
        ]);
        if (sec && sec.id) setSectionsConfig(sec);
        if (prm && (prm.id || prm.title_part1)) setEditorialPromoForm(prm);
        if (sty && (sty.id || sty.heading)) setBrandStoryForm(sty);
        if (igs && Array.isArray(igs)) setInstagramPosts(igs);
      }
  ...
  const handleToggleCollection = (colId) => {
    const current = productForm.collection_ids || [];
    const updated = current.includes(colId) ? current.filter(id => id !== colId) : [...current, colId];
  ...
  // Dynamic Hero Slide CRUD Handlers
  ...
  const handleHeroSlideSubmit = async (e) => {
    ...
    try {
      const url = editingHeroSlide
        ? getApiUrl(`/api/admin/hero-slides/${editingHeroSlide.id}`)
      ...
      if (res.ok) {
        ...
        const updatedSlides = await safeFetchJson('/api/admin/hero-slides');
        if (updatedSlides && Array.isArray(updatedSlides)) setHeroSlides(updatedSlides);
        if (showToast) showToast('success', 'Slide Saved Live!', editingHeroSlide ? 'Hero slide updated successfully.' : 'New hero slide added.');
      } else {
  ...
  const handleDeleteHeroSlide = (slideId) => {
    askConfirmation({
      ...
      message: 'Are you sure you want to delete this rotating hero slide from your storefront?',
      confirmText: 'Delete Slide',
      danger: true,
      onConfirm: async () => {
        try {
          const res = await fetch(getApiUrl(`/api/admin/hero-slides/${slideId}`), { method: 'DELETE' });
          if (res.ok) {
            setHeroSlides(prev => prev.filter(s => s.id !== slideId));
            if (showToast) showToast('info', 'Slide Deleted', 'Hero slide removed successfully.');
          }
        } catch (err) {
          if (showToast) showToast('error', 'Error', err.message);
        }
  ...
  const handleToggleHeroSlideActive = async (slide) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (showToast) showToast('success', 'Status Updated', `Slide is now ${updatedStatus === 1 ? 'ACTIVE' : 'INACTIVE'}.`);
      }
    } catch (err) {
      if (showToast) showToast('error', 'Error', err.message);
    }
  ...
  // Dynamic Instagram Posts CRUD Handlers
  const handleOpenAddInstagramPost = () => {
    setEditingInstagramPost(null);
    setInstagramPostForm({
      title: '',
      image_url: '',
      post_url: 'https://www.instagram.com/valuelife_essentials/?hl=en',
  ...
  const handleOpenEditInstagramPost = (post) => {
    setEditingInstagramPost(post);
    setInstagramPostForm({
      title: post.title || '',
      image_url: post.image_url || '',
      post_url: post.post_url || 'https://www.instagram.com/valuelife_essentials/?hl=en',
      sort_order: post.sort_order || 0,
      is_active: post.is_active !== undefined ? Number(post.is_active) : 1
    });
    setShowInstagramPostModal(true);
  };
  ...
  const handleInstagramPostSubmit = async (e) => {
    e.preventDefault();
    try {
      const url = editingInstagramPost
        ? getApiUrl(`/api/admin/instagram-posts/${editingInstagramPost.id}`)
        : getApiUrl('/api/admin/instagram-posts');
      const method = editingInstagramPost ? 'PUT' : 'POST';

      const res = await fetch(url, {
        method,
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(instagramPostForm)
      });

      if (res.ok) {
        setShowInstagramPostModal(false);
        const updatedPosts = await safeFetchJson('/api/admin/instagram-posts');
        if (updatedPosts && Array.isArray(updatedPosts)) setInstagramPosts(updatedPosts);
        if (showToast) showToast('success', 'Instagram Post Saved Live!', editingInstagramPost ? 'Post updated successfully.' : 'New post added to Instagram gallery.');
      } else {
        const errData = await res.json().catch(() => ({}));
        if (showToast) showToast('error', 'Failed to Save', errData.error || 'Server error occurred.');
      }
  ...
  const handleDeleteInstagramPost = (postId) => {
    askConfirmation({
      title: 'Delete Instagram Post?',
      message: 'Are you sure you want to remove this post from your storefront Instagram gallery?',
      confirmText: 'Delete Post',
      danger: true,
      onConfirm: async () => {
        try {
          const res = await fetch(getApiUrl(`/api/admin/instagram-posts/${postId}`), { method: 'DELETE' });
          if (res.ok) {
            setInstagramPosts(prev => prev.filter(p => p.id !== postId));
            if (showToast) showToast('info', 'Post Deleted', 'Instagram post removed.');
          }
        } catch (err) {
          if (showToast) showToast('error', 'Error', err.message);
        }
  ...
  const handleToggleInstagramPostActive = async (post) => {
    const updatedStatus = Number(post.is_active) === 1 ? 0 : 1;
    try {
      const res = await fetch(getApiUrl(`/api/admin/instagram-posts/${post.id}`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...post, is_active: updatedStatus })
      });
      if (res.ok) {
        setInstagramPosts(prev => prev.map(p => p.id === post.id ? { ...p, is_active: updatedStatus } : p));
        if (showToast) showToast('success', 'Status Updated', `Post is now ${updatedStatus === 1 ? 'ACTIVE' : 'INACTIVE'}.`);
      }
    } catch (err) {
      if (showToast) showToast('error', 'Error', err.message);
    }
  ...
  // Dynamic Editorial Promo Banner Submit Handler
  const handleEditorialPromoSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(getApiUrl('/api/admin/editorial-promo'), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(editorialPromoForm)
      });
      if (res.ok) {
        if (showToast) showToast('success', 'Editorial Promo Saved Live!', 'Promotional banner details updated.');
      } else {
        if (showToast) showToast('error', 'Failed to Save', 'Could not update editorial banner.');
      }
    } catch (err) {
      if (showToast) showToast('error', 'Error', err.message);
    }
  ...
  // Dynamic Brand Story Section Submit Handler
  const handleBrandStorySubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(getApiUrl('/api/admin/brand-story'), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(brandStoryForm)
      });
      if (res.ok) {
        if (showToast) showToast('success', 'Brand Story Saved Live!', 'Our story section updated successfully.');
      } else {
        if (showToast) showToast('error', 'Failed to Save', 'Could not update brand story.');
      }
    } catch (err) {
      if (showToast) showToast('error', 'Error', err.message);
    }
  ...
  const salesChartData = {
    labels: salesChartLabels.length ? salesChartLabels : ['07 Aug', '08 Aug', '09 Aug', '10 Aug', '11 Aug', '12 Aug', '13 Aug'],
    datasets: [{
      label: 'Daily Revenue (₹)',
      data: analytics?.salesChart?.map(d => d.revenue) || [649, 899, 1250, 749, 1120, 1450, 980],
      borderColor: '#10b981',
              ...
              {openSubmenus.marketing && (
                <div className="pl-3.5 space-y-1 border-l-2 border-slate-800 ml-3.5">
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all ${
                      activeTab === 'instagram' ? 'bg-emerald-600 text-white shadow-md font-extrabold' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
                    }`}
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <Sparkles size={16} className="text-pink-400 flex-shrink-0" /> 
                      <span className="truncate whitespace-nowrap text-xs font-bold">📸 Instagram Feed (6-Grid)</span>
                    </div>
                    <span className="bg-slate-800 text-slate-400 text-[10px] px-2 py-0.5 rounded-md border border-slate-700 font-extrabold flex-shrink-0 whitespace-nowrap">
                  ...
                  <button 
                    ...
                    className={`w-full flex items-center justify-between px-3 py-2.5 rounded-xl transition-all ${
                      activeTab === 'editorial' ? 'bg-emerald-600 text-white shadow-md font-extrabold' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
                    }`}
                  >
                    <div className="flex items-center gap-2.5 min-w-0">
                      <Layers size={16} className="text-amber-400 flex-shrink-0" /> 
                      <span className="truncate whitespace-nowrap text-xs font-bold">📖 Editorial Promo & Story</span>
                    </div>
                  </button>
        ...
        <main className="p-6 space-y-6 flex-1">
                          ...
                          <button 
                            onClick={() => {
                              ...
                              setProductForm({
                                ...
                                discount_inr: p.discount_inr || p.price_inr,
                                discount_usd: p.discount_usd || p.price_usd,
                                compare_price_inr: p.compare_price_inr || p.price_inr + 100,
                                compare_price_usd: p.compare_price_usd || p.price_usd + 3,
                                cost_per_item_inr: p.cost_per_item_inr || 150,
                                cost_per_item_usd: p.cost_per_item_usd || 4,
                                barcode: p.barcode || '',
                                stock: p.stock,
          ...
          {activeTab === 'hero' && (
            <div className="space-y-6">
              ...
              {/* ROTATING HERO SLIDES CAROUSEL MANAGER */}
              <div className="bg-slate-900 p-6 rounded-2xl border border-slate-800 shadow-md space-y-5">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-slate-800 pb-4">
                  <div>
                    <h4 className="text-base font-black text-white flex items-center gap-2 font-['Outfit']">
                      🎡 Rotating Hero Carousel Slides ({heroSlides.length})
                    </h4>
                    <p className="text-xs text-slate-400">
                      Manage multiple dynamic rotating slides shown on the storefront. All images, headlines, taglines, quotes, and buttons are dynamic from database.
                    </p>
                  </div>
                  <button
                    type="button"
                    onClick={handleOpenAddHeroSlide}
                    className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold text-xs flex items-center gap-2 shadow-lg transition-all cursor-pointer"
                  >
                    <Plus size={16} />
                ...
                {heroSlides.length === 0 ? (
                  <div className="p-8 text-center bg-slate-950/60 rounded-xl border border-slate-800 space-y-2">
                    ...
                    <p className="text-sm font-bold text-slate-300">No slides found in database</p>
                    <p className="text-xs text-slate-500">Click "Add New Slide" to create your first dynamic storefront slide.</p>
                  </div>
                ) : (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      >
                          ...
                          <div className="flex items-center gap-1.5">
                            <button
                              ...
                              className={`text-[10px] font-bold px-2 py-0.5 rounded cursor-pointer ${
                                Number(slide.is_active) === 1 
                                  ? 'bg-emerald-950 text-emerald-400 border border-emerald-800' 
                                  : 'bg-slate-800 text-slate-400 border border-slate-700'
                              }`}
                            >
                              {Number(slide.is_active) === 1 ? 'ACTIVE' : 'INACTIVE'}
                            </button>
                            <button
                              type="button"
                            ...
                            <button
                              ...
                              className="p-1.5 bg-rose-950/60 hover:bg-rose-900 text-rose-400 rounded-lg cursor-pointer"
                              title="Delete Slide"
                            >
                              <Trash2 size={14} />
                            </button>
                        ...
                        <div>
                          <h5 className="text-sm font-extrabold text-white font-serif">
                            {slide.title_part1} {slide.title_part2 && <span className="text-emerald-400 italic font-normal">{slide.title_part2}</span>}
                          </h5>
                        ...
                        <div className="flex items-center gap-3 pt-1">
                          ...
                          <div className="min-w-0 text-[11px] text-slate-400 space-y-0.5 truncate">
                            <div><strong className="text-slate-300">CTA:</strong> {slide.cta_text || 'Shop Now'} → <span className="font-mono text-[10px] text-slate-500">{slide.cta_link || '/products'}</span></div>
                            {slide.script_quote && <div className="italic text-emerald-400/80 truncate">"{slide.script_quote}"</div>}
                          </div>
                        </div>
                      </div>
                    ))}
          ...
          {/* TAB: INSTAGRAM FEED 6-GRID STUDIO */}
          {activeTab === 'instagram' && (
            <div className="space-y-6">
              {/* HEADER */}
              <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-md flex flex-wrap justify-between items-center gap-4">
                <div>
                  <h3 className="text-lg font-extrabold text-white flex items-center gap-2 font-['Outfit']">
                    📸 Instagram Feed & Gallery Studio ({instagramPosts.length} Posts)
                  </h3>
                  <p className="text-xs text-slate-400">
                    Manage the 6-grid Instagram storefront showcase. Add, edit, or delete posts, upload images, and set custom post links.
                  </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={handleOpenAddInstagramPost}
                    className="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold text-xs flex items-center gap-2 shadow-lg transition-all cursor-pointer"
                  >
                    <Plus size={16} />
              ...
              {instagramPosts.length === 0 ? (
                <div className="p-12 text-center bg-slate-900 rounded-2xl border border-slate-800 space-y-3">
                  <Sparkles className="mx-auto text-pink-400" size={36} />
                  <h4 className="text-base font-bold text-white">No Instagram Posts Found</h4>
                  <p className="text-xs text-slate-400">Click "Add New Post" to publish images to the storefront Instagram gallery.</p>
                </div>
              ) : (
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
                  {instagramPosts.map((post, idx) => (
                    <div 
                      key={post.id || idx}
                      className="bg-slate-900 rounded-2xl border border-slate-800 overflow-hidden shadow-md group flex flex-col justify-between"
                    >
                      <div className="relative aspect-square overflow-hidden bg-slate-950">
                        <img 
                          src={resolveImgUrl(post.image_url)} 
                          alt={post.title} 
                          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
                        />
                        <div className="absolute top-2 right-2 flex items-center gap-1 bg-slate-950/80 backdrop-blur-sm p-1 rounded-lg">
                          <button
                            type="button"
                            onClick={() => handleOpenEditInstagramPost(post)}
                      ...
                      <div className="p-3 space-y-2 text-xs flex-1 flex flex-col justify-between">
                        <div>
                          <span className="font-bold text-white block truncate">{post.title || 'Instagram Post'}</span>
                          <a 
                            href={post.post_url || 'https://www.instagram.com/valuelife_essentials/?hl=en'} 
                            target="_blank" 
                            rel="noreferrer"
                            className="text-[10px] text-emerald-400 hover:underline truncate block flex items-center gap-1 mt-0.5"
                          >
                        ...
                        <div className="flex justify-between items-center pt-2 border-t border-slate-800">
                          <span className="text-[10px] text-slate-500 font-mono">#{post.sort_order || idx + 1}</span>
                          <button
                            type="button"
                            onClick={() => handleToggleInstagramPostActive(post)}
                          ...
                          >
                            {Number(post.is_active) === 1 ? 'ON' : 'OFF'}
                          </button>
                        </div>
                      </div>
                    </div>
          ...
          {/* TAB: EDITORIAL PROMO & BRAND STORY STUDIO */}
          {activeTab === 'editorial' && (
            <div className="space-y-6">
              {/* HEADER */}
              <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-md">
                <h3 className="text-lg font-extrabold text-white flex items-center gap-2 font-['Outfit']">
                  📖 Editorial Promo Banner & Brand Story Studio
                </h3>
                <p className="text-xs text-slate-400">
                  Manage both key brand narrative sections on your homepage: the Editorial Promo Banner ("Pure Products Happier Lives") and the Brand Story section ("Our Story").
                </p>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* 1. EDITORIAL PROMO BANNER FORM */}
                <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-md space-y-4">
                  <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                    <h4 className="font-extrabold text-sm text-white flex items-center gap-2">
                      <Sparkles size={16} className="text-amber-400" />
                      1. Editorial Promo Banner
                    </h4>
                  ...
                  <form onSubmit={handleEditorialPromoSubmit} className="space-y-3 text-xs">
                    <div>
                      <label className="block text-slate-300 font-bold mb-1">Badge Pill Text</label>
                      <input 
                        type="text"
                        value={editorialPromoForm.badge_text || ''}
                        onChange={(e) => setEditorialPromoForm({ ...editorialPromoForm, badge_text: e.target.value })}
                        className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white font-bold"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-slate-300 font-bold mb-1">Headline Part 1</label>
                        <input 
                          type="text"
                          value={editorialPromoForm.title_part1 || ''}
                          onChange={(e) => setEditorialPromoForm({ ...editorialPromoForm, title_part1: e.target.value })}
                          className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white font-bold"
                          placeholder="e.g. Pure Products"
                        />
                      </div>
                      <div>
                        <label className="block text-slate-300 font-bold mb-1">Headline Part 2 (Italic)</label>
                        <input 
                          type="text"
                          value={editorialPromoForm.title_part2 || ''}
                          onChange={(e) => setEditorialPromoForm({ ...editorialPromoForm, title_part2: e.target.value })}
                          className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white font-bold"
                          placeholder="e.g. Happier Lives"
                        />
                      </div>
                    </div>

                    <div>
                      <label className="block text-slate-300 font-bold mb-1">Subtitle / Offer Description</label>
                      <input 
                        type="text"
                        value={editorialPromoForm.subtitle || ''}
                        onChange={(e) => setEditorialPromoForm({ ...editorialPromoForm, subtitle: e.target.value })}
                        className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                        placeholder="e.g. Flat 20% Off on Natural Essentials & Certified Organics"
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input 
                          type="text"
                          value={editorialPromoForm.cta_text || ''}
                          onChange={(e) => setEditorialPromoForm({ ...editorialPromoForm, cta_text: e.target.value })}
                          className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white font-bold"
                          placeholder="e.g. Shop the Collection"
                        />
                      </div>
                      <div>
                        <label className="block text-slate-300 font-bold mb-1">CTA Button Target Link</label>
                        <input 
                          type="text"
                          value={editorialPromoForm.cta_link || ''}
                          onChange={(e) => setEditorialPromoForm({ ...editorialPromoForm, cta_link: e.target.value })}
                          className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white font-mono"
                          placeholder="e.g. /offers or /products"
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-slate-300 font-bold mb-1">Script Quote (Cursive)</label>
                        <input 
                          type="text"
                          value={editorialPromoForm.script_quote || ''}
                          onChange={(e) => setEditorialPromoForm({ ...editorialPromoForm, script_quote: e.target.value })}
                          className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                          placeholder="e.g. Nature Nurtures You"
                        />
                      </div>
                      <div>
                        <label className="block text-slate-300 font-bold mb-1">Quote Subtext</label>
                        <input 
                          type="text"
                          value={editorialPromoForm.quote_subtext || ''}
                          onChange={(e) => setEditorialPromoForm({ ...editorialPromoForm, quote_subtext: e.target.value })}
                          className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                    ...
                    <ImageUploader 
                      label="Banner Photography Image *"
                      value={editorialPromoForm.image_url || ''}
                      onChange={(newUrl) => setEditorialPromoForm({ ...editorialPromoForm, image_url: newUrl })}
                      placeholder="https://images.unsplash.com/..."
                    />

                    <button 
                      type="submit"
                      className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl shadow-lg transition-all cursor-pointer"
                    >
                      Save Editorial Promo Banner Live
                    </button>
                  </form>
                </div>
                ...
                <div className="bg-slate-900 p-5 rounded-2xl border border-slate-800 shadow-md space-y-4">
                  ...
                  <form onSubmit={handleBrandStorySubmit} className="space-y-3 text-xs">
                    <div>
                      <label className="block text-slate-300 font-bold mb-1">Badge Tagline</label>
                      <input 
                        type="text"
                        value={brandStoryForm.badge_text || ''}
                        onChange={(e) => setBrandStoryForm({ ...brandStoryForm, badge_text: e.target.value })}
                        className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white font-bold"
                    ...
                    <div>
                      <label className="block text-slate-300 font-bold mb-1">Section Heading</label>
                      <input 
                        type="text"
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-slate-300 font-bold mb-1">Image Overlay Title</label>
                        <input 
                          type="text"
                          value={brandStoryForm.overlay_title || ''}
                          onChange={(e) => setBrandStoryForm({ ...brandStoryForm, overlay_title: e.target.value })}
                          className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white font-bold"
                          placeholder="e.g. A Healthier"
                        />
                      </div>
                      <div>
                        <label className="block text-slate-300 font-bold mb-1">Image Overlay Subtitle</label>
                        <input 
                          type="text"
                          value={brandStoryForm.overlay_subtitle || ''}
                          onChange={(e) => setBrandStoryForm({ ...brandStoryForm, overlay_subtitle: e.target.value })}
                          className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white font-bold"
                          placeholder="e.g. Tomorrow Together"
                        />
                      </div>
                    </div>

                    <div>
                      <label className="block text-slate-300 font-bold mb-1">Story Narrative Description</label>
                      <textarea 
                        rows={4}
                        value={brandStoryForm.description || ''}
                        onChange={(e) => setBrandStoryForm({ ...brandStoryForm, description: e.target.value })}
                        className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                        placeholder="Write your brand mission and origin story here..."
                      />
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input 
                          type="text"
                          value={brandStoryForm.cta_text || ''}
                          onChange={(e) => setBrandStoryForm({ ...brandStoryForm, cta_text: e.target.value })}
                          className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white font-bold"
                      ...
                      <div>
                        ...
                        <input 
                          type="text"
                          value={brandStoryForm.cta_link || ''}
                          onChange={(e) => setBrandStoryForm({ ...brandStoryForm, cta_link: e.target.value })}
                          className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white font-mono"
                    ...
                    <ImageUploader 
                      ...
                      value={brandStoryForm.image_url || ''}
                      onChange={(newUrl) => setBrandStoryForm({ ...brandStoryForm, image_url: newUrl })}
                      placeholder="https://images.unsplash.com/..."
                    />
            ...
            <form onSubmit={handleProductSubmit} className="grid grid-cols-1 lg:grid-cols-3 gap-8 text-xs">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-slate-850 p-4 rounded-xl border border-slate-800 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-emerald-400 font-bold mb-1">Price INR (₹) *</label>
                      <input 
                    ...
                    <div>
                      <label className="block text-blue-400 font-bold mb-1">Price USD ($) *</label>
                      <input 
                        type="number" required placeholder="$ 0.00"
                  ...
                  <div className="grid grid-cols-2 gap-4 pt-2 border-t border-slate-800">
                    <div>
                      <label className="block text-slate-400 mb-1">Compare-at Price INR (₹)</label>
                      <input 
                        type="number" placeholder="₹ Original Price"
                        value={productForm.compare_price_inr ?? ''}
                        onChange={(e) => setProductForm({ ...productForm, compare_price_inr: Number(e.target.value) })}
                        className="w-full p-2 bg-slate-800 border border-slate-700 rounded-lg text-slate-300"
                      />
                    </div>
                    <div>
                      <label className="block text-slate-400 mb-1">Cost per item INR (₹)</label>
                      <input 
                        type="number" placeholder="₹ Supplier Cost"
                        value={productForm.cost_per_item_inr ?? ''}
                        onChange={(e) => setProductForm({ ...productForm, cost_per_item_inr: Number(e.target.value) })}
                        className="w-full p-2 bg-slate-800 border border-slate-700 rounded-lg text-slate-300"
                      />
                    </div>
                  </div>
            ...
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <div>
                ...
                <h3 className="font-extrabold text-base text-white">
                  {editingHeroSlide ? 'Edit Rotating Hero Slide' : 'Add New Rotating Hero Slide'}
                </h3>
              </div>
              <button onClick={() => setShowHeroSlideModal(false)}><XCircle size={24} /></button>
            </div>

            <form onSubmit={handleHeroSlideSubmit} className="space-y-3.5 text-xs">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block font-bold text-slate-300 mb-1">Headline Part 1 *</label>
                  <input 
                    type="text" required
                    value={heroSlideForm.title_part1}
                    onChange={(e) => setHeroSlideForm({ ...heroSlideForm, title_part1: e.target.value })}
                    className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                ...
                <div>
                  ...
                  <input 
                    type="text"
                    value={heroSlideForm.title_part2}
                    onChange={(e) => setHeroSlideForm({ ...heroSlideForm, title_part2: e.target.value })}
              ...
              <div>
                ...
                <input 
                  type="text" required
                  value={heroSlideForm.tagline}
                  onChange={(e) => setHeroSlideForm({ ...heroSlideForm, tagline: e.target.value })}
                  className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white uppercase font-bold"
                  placeholder="e.g. NATURAL • HEALTHY • SUSTAINABLE"
                />
              </div>
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                  placeholder="Discover natural, healthy and premium products..."
                />
              </div>
              ...
              <div>
                ...
                <input 
                  type="text"
                  value={heroSlideForm.script_quote}
                  onChange={(e) => setHeroSlideForm({ ...heroSlideForm, script_quote: e.target.value })}
                  className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                  placeholder="Good Products, Brighter Days."
                />
              </div>

              <ImageUploader 
                label="Primary Packaging Image *"
                value={heroSlideForm.packaging_img}
                onChange={(url) => setHeroSlideForm({ ...heroSlideForm, packaging_img: url })}
                placeholder="Upload primary product image or paste image link..."
              />

              <ImageUploader 
                label="Secondary Floating Visual Image (Optional)"
                value={heroSlideForm.jars_img}
                onChange={(url) => setHeroSlideForm({ ...heroSlideForm, jars_img: url })}
                placeholder="Upload floating secondary accent image..."
              />
              ...
              <div className="grid grid-cols-2 gap-3 pt-2">
                <div>
                  ...
                  <input 
                    ...
                    onChange={(e) => setHeroSlideForm({ ...heroSlideForm, sort_order: Number(e.target.value) })}
                    className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                  />
                </div>
                <div>
                  ...
                  <select 
                    value={heroSlideForm.is_active}
                    onChange={(e) => setHeroSlideForm({ ...heroSlideForm, is_active: Number(e.target.value) })}
                    className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                  >
              ...
              <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl shadow-lg transition-all cursor-pointer">
                {editingHeroSlide ? 'Save & Update Slide' : 'Create Hero Slide'}
              </button>
            </form>
          </div>
        </div>
      ...
      {showInstagramPostModal && (
        <div className="drawer-overlay flex items-center justify-center p-4 z-50">
          <div className="bg-slate-900 border border-slate-800 text-slate-100 rounded-2xl max-w-md w-full p-6 space-y-4 shadow-2xl">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <div>
                <span className="text-[10px] font-black uppercase text-pink-400 tracking-wider">INSTAGRAM FEED</span>
                <h3 className="font-extrabold text-base text-white">
                  {editingInstagramPost ? 'Edit Instagram Post' : 'Add Instagram Post'}
                </h3>
              </div>
              <button onClick={() => setShowInstagramPostModal(false)}><XCircle size={24} /></button>
            </div>

            <form onSubmit={handleInstagramPostSubmit} className="space-y-3.5 text-xs">
              <div>
                <label className="block font-bold text-slate-300 mb-1">Post Title / Caption *</label>
                <input 
                  type="text" required
                  value={instagramPostForm.title}
                  onChange={(e) => setInstagramPostForm({ ...instagramPostForm, title: e.target.value })}
                  className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                  placeholder="e.g. Chia Smoothie Bowl"
                />
              </div>

              <ImageUploader 
                label="Instagram Post Photo *"
                value={instagramPostForm.image_url}
                onChange={(url) => setInstagramPostForm({ ...instagramPostForm, image_url: url })}
                placeholder="Upload photo or paste URL..."
              />

              <div>
                <label className="block font-bold text-slate-300 mb-1">Direct Instagram Link</label>
                <input 
                  type="url"
                  value={instagramPostForm.post_url}
                  onChange={(e) => setInstagramPostForm({ ...instagramPostForm, post_url: e.target.value })}
                  className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white font-mono"
                  placeholder="https://www.instagram.com/valuelife_essentials/?hl=en"
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input 
                    type="number"
                    value={instagramPostForm.sort_order}
                    onChange={(e) => setInstagramPostForm({ ...instagramPostForm, sort_order: Number(e.target.value) })}
                    className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                  />
                </div>
                <div>
                  <label className="block font-bold text-slate-300 mb-1">Active Status</label>
                  <select 
                    value={instagramPostForm.is_active}
                    onChange={(e) => setInstagramPostForm({ ...instagramPostForm, is_active: Number(e.target.value) })}
                    className="w-full p-2.5 bg-slate-800 border border-slate-700 rounded-lg text-white"
                  >
              ...
              <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl shadow-lg transition-all cursor-pointer">
                {editingInstagramPost ? 'Save & Update Post' : 'Publish Instagram Post'}
              </button>
            </form>
          </div>
        </div>
