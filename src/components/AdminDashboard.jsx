...
export default function AdminDashboard({ onExitAdmin, showToast, sectionsConfig: propSectionsConfig, onUpdateSectionsConfig, settings: propSettings, onUpdateSettings }) {
  ...
  const fetchAdminData = async () => {
    const [
      prods, cats, colls, revs, bans, cpns, ords, usrs, sets, pgs, flts, hero, thm, sec, meds
    ] = await Promise.all([
      ...
      safeFetchJson('/api/pages'),
      safeFetchJson('/api/filter-groups'),
      safeFetchJson('/api/hero-config'),
      safeFetchJson('/api/theme-config'),
  ...
  const isDuplicateSku = React.useMemo(() => {
    ...
    return products.some(p => p.id !== productForm.id && (
      p.sku?.toUpperCase() === cleanSku || 
      p.variants?.some(v => v.sku?.toUpperCase() === cleanSku)
    ));
  }, [productForm.sku, productForm.id, products]);

  const handleProductSubmit = async (e) => {
    e.preventDefault();

    if (isDuplicateSku) {
      if (showToast) showToast('error', 'Duplicate SKU Error', `SKU ID "${productForm.sku}" is already assigned to another product. Please use a unique SKU ID.`);
      return;
    }
    ...
    try {
      ...
      const payload = {
        ...
        sku: productForm.sku || `VLE-PROD-${Math.floor(100 + Math.random() * 9000)}`
      };
                                ...
                                <button 
                                  onClick={async () => {
                                    if (window.confirm(`Delete image "${item.filename}" from server?`)) {
                                      await fetch(getApiUrl(`/api/media/${encodeURIComponent(item.filename)}`), { method: 'DELETE' });
                                      fetchAdminData();
                                  ...
                                  <button 
                                    onClick={() => {
                                      ...
                                      const pImages = Array.isArray(p.images) && p.images.length > 0 
                                        ? p.images 
                                        : (p.image_url ? [p.image_url] : (p.thumbnail ? [p.thumbnail] : []));
                                      setProductForm({ ...p, images: pImages, variants: p.variants || [] });
                                      setShowProductModal(true);
                                    }}
                          ...
                          {productForm.variants.map((v, vIdx) => (
                            <tr key={vIdx} className="hover:bg-slate-850/70 transition-colors">
                              <td className="p-2.5">
                                <div className="flex items-center gap-2">
                                  <img 
                                    src={resolveImgUrl(v.image_url || productForm.images?.[0])} 
                                    alt={v.variant_name || 'Variant'}
                                    onError={(e) => {
                                      e.target.onerror = null;
                                      e.target.src = 'https://images.unsplash.com/photo-1585320806297-9794b3e4eeae?auto=format&fit=crop&w=100&q=80';
                                    }}
                                    className="w-10 h-10 object-cover rounded-lg border border-slate-700 bg-white shrink-0 shadow-sm"
                                  />
                                  <label className="cursor-pointer bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white px-2 py-1 rounded text-[10px] font-bold border border-slate-700 transition-colors inline-block">
                                    <span>Upload</span>
                                    <input 
                                      type="file" 
                                      accept="image/*"
                                      ...
                                      onChange={async (e) => {
                                        ...
                                        try {
                                          ...
                                          if (data.imageUrl) {
                                            ...
                                            updated[vIdx].image_url = data.imageUrl;
                                            setProductForm({ ...productForm, variants: updated });
                                            if (showToast) showToast('success', 'Variant Image Updated', 'Uploaded custom image for variant.');
                                          }
                                        } catch (err) {
                                          if (showToast) showToast('error', 'Upload Failed', 'Failed to upload variant image.');
                                        }
                                      }}
                                    />
                                  </label>
                ...
                <div className="bg-slate-850 p-4 rounded-xl border border-slate-800 space-y-4">
                  ...
                  <div className="bg-white text-gray-900 p-4 rounded-xl space-y-1 shadow-inner border border-gray-200">
                    <div className="text-[11px] text-gray-500 flex items-center gap-1 font-mono">
                      <span>VALUELIFE ESSENTIALS</span>
                      <span>›</span>
                      <span>https://valuelifeessentials.com</span>
                      <span>›</span>
