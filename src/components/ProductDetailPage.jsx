...
import { 
  ...
  ChevronRight, Award, Zap, ThumbsUp, HelpCircle, Check, MapPin, Truck as DeliveryTruck, Layers, Plus, X
} from 'lucide-react';

const resolveImgUrl = (url, fallback = 'https://images.unsplash.com/photo-1585320806297-9794b3e4eeae?w=500') => {
...
export default function ProductDetailPage({ 
  ...
  const rawPrice = isINR ? (Number(targetItem.price_inr) || 0) : (Number(targetItem.price_usd) || 0);
  const rawDiscount = isINR 
    ? (targetItem.discount_inr !== undefined && targetItem.discount_inr !== null && targetItem.discount_inr > 0 ? Number(targetItem.discount_inr) : null)
    : (targetItem.discount_usd !== undefined && targetItem.discount_usd !== null && targetItem.discount_usd > 0 ? Number(targetItem.discount_usd) : null);
  const rawCompare = isINR
    ? (targetItem.compare_price_inr !== undefined && targetItem.compare_price_inr !== null && targetItem.compare_price_inr > 0 ? Number(targetItem.compare_price_inr) : (productData.compare_price_inr ? Number(productData.compare_price_inr) : null))
    : (targetItem.compare_price_usd !== undefined && targetItem.compare_price_usd !== null && targetItem.compare_price_usd > 0 ? Number(targetItem.compare_price_usd) : (productData.compare_price_usd ? Number(productData.compare_price_usd) : null));
      ...
      <div className="max-w-7xl mx-auto px-4 pt-6 space-y-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="space-y-4">
            <div className="w-full h-96 sm:h-[460px] bg-gray-50 rounded-3xl overflow-hidden border border-gray-200 relative group shadow-sm flex items-center justify-center p-2">
              <img 
                ...
                {allProductImages.map((imgUrl, idx) => (
                  <button 
                    ...
                    className={`w-20 h-20 rounded-2xl overflow-hidden border-2 transition-all flex-shrink-0 bg-white ${
                      (selectedImage || allProductImages[0]) === imgUrl ? 'border-[#2d6a4f] ring-2 ring-[#2d6a4f]/20 shadow-md scale-105' : 'border-gray-200 hover:border-gray-400 opacity-80 hover:opacity-100'
                    }`}
                  >
                    <img 
