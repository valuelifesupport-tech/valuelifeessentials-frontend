...
import { RefreshCw, Truck, X, CheckCircle2, Clock, MapPin, AlertCircle, ExternalLink } from 'lucide-react';
import { resolveImgUrl, getApiUrl } from '../../../api/config';
...
export default function ProfileOrdersTab({
  ...
  const handleOpenTrackingModal = async (order) => {
    ...
    try {
      ...
      if (data.success && data.tracking) {
        ...
        setTrackingError(data.error || 'Tracking details are being generated by the courier. Please check back shortly.');
      }
    } catch (err) {
      setTrackingError('Unable to fetch live tracking information at this moment.');
    } finally {
  ...
  return (
    <div className="bg-white p-6 rounded-3xl border border-gray-200/80 shadow-sm space-y-6" data-reticle-target="user-profile-orders-tab">
            ...
            {paginatedOrders.map((order) => {
              ...
              return (
                <div key={order.id} className="p-5 bg-gray-50 rounded-2xl border border-gray-200/80 space-y-3 hover:border-gray-300 transition-all" data-reticle-target={`profile-order-${order.id}`}>
                  ...
                  {(order.courier_name || order.shiprocket_awb || order.tracking_number) && (
                    <div className="p-3 bg-blue-50/90 rounded-xl border border-blue-200 flex flex-wrap justify-between items-center text-xs gap-2">
                      <div className="flex items-center gap-2 flex-wrap">
                        <Truck size={16} className="text-blue-600 shrink-0" />
                        <span className="font-extrabold text-blue-950">
                          {order.courier_name || order.shiprocket_courier_name || 'Express Courier'}
                        </span>
                        {(order.shiprocket_awb || order.tracking_number) && (
                          <span className="font-mono bg-blue-100 text-blue-900 px-2 py-0.5 rounded font-bold text-[11px]">
                            AWB: #{order.shiprocket_awb || order.tracking_number}
                          </span>
                        )}
                      ...
                      <button
                        ...
                        className="bg-blue-600 hover:bg-blue-700 text-white font-extrabold px-3 py-1.5 rounded-lg flex items-center gap-1.5 shadow-sm cursor-pointer transition-colors text-xs ml-auto"
                        data-reticle-target={`track-order-btn-${order.id}`}
                      >
      ...
      {/* CUSTOMER LIVE SHIPMENT TRACKING MODAL */}
      {activeTrackingOrder && (
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-gray-100 space-y-4 max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center border-b pb-3">
              <div>
                ...
                <h3 className="text-base font-extrabold text-gray-900 font-mono">
                  Order #{activeTrackingOrder.order_number}
                </h3>
              </div>
              <button
                type="button"
                onClick={() => { setActiveTrackingOrder(null); setTrackingData(null); }}
            ...
            {loadingTracking ? (
              <div className="py-12 text-center text-gray-500 text-xs space-y-2">
                ...
                <p className="font-bold">Contacting Shiprocket Courier Network...</p>
              </div>
            ) : trackingError ? (
              <div className="py-6 text-center space-y-2 bg-amber-50 rounded-2xl border border-amber-200 p-4">
                <AlertCircle size={24} className="mx-auto text-amber-600" />
                <p className="text-xs font-bold text-amber-900">{trackingError}</p>
                <p className="text-[11px] text-amber-700">
                  Your package has been booked with {activeTrackingOrder.courier_name || 'courier'}. Live tracking updates typically appear once the courier scans the package at the first distribution hub.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <div className="bg-gray-50 p-4 rounded-2xl border border-gray-200/80 grid grid-cols-2 gap-3 text-xs">
                  <div>
                    <span className="text-[10px] font-bold text-gray-400 uppercase block">Courier Partner</span>
                    <span className="font-extrabold text-gray-900">
                      {activeTrackingOrder.shiprocket_courier_name || activeTrackingOrder.courier_name || trackingData?.courier_name || 'Express Courier'}
                    </span>
                  </div>
                  <div>
                    <span className="text-[10px] font-bold text-gray-400 uppercase block">AWB Tracking #</span>
                    <span className="font-extrabold text-emerald-700 font-mono">
                      {activeTrackingOrder.shiprocket_awb || activeTrackingOrder.tracking_number || trackingData?.awb_code}
                    </span>
                  </div>
                  <div>
                    ...
                    <span className="font-extrabold text-blue-700 uppercase">
                      {trackingData?.current_status || activeTrackingOrder.shiprocket_status || 'In Transit'}
                    </span>
                  </div>
                  {trackingData?.etd && (
                    <div>
                ...
                <div className="space-y-2">
                  ...
                  {trackingData?.scans && trackingData.scans.length > 0 ? (
                    ...
                    <div className="p-4 bg-emerald-50/60 rounded-xl border border-emerald-200 text-xs text-emerald-950 flex items-center gap-2">
                      <CheckCircle2 size={16} className="text-emerald-600 shrink-0" />
                      <span>Shipment manifested and assigned to courier. In transit to destination facility.</span>
                    </div>
                  )}
                </div>
              </div>
